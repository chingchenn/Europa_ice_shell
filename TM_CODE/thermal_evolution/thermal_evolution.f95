...
!                                                                                              |
!  Compile: gfortran -o thermal_evolution -O4 thermal_evolution_mod.f95 thermal_evolution.f95  |
...
!---> Now calculates time evolution of radial structure
      ...
      do 200 i=1,npt
         ...
         open(90,file='heat.902',status='unknown')
         write(90,'(1x,f8.4,2x,f7.2)') time/1.0d9,r_bot/1.0d6
         z_bot = R_tot - r_bot
         Volume_iceI = 4.0d0*pi*(R_tot**3.0d0 - r_bot**3.0d0)/3.0d0
         open(80,file='heat.802',status='unknown')
         write(80,'(1x,f8.4,2x,f7.2,2x,f8.4,2x)') time/1.0d9,r_bot/1.0d6,Volume_iceI/1.0d18
         if (heat_param == 1) then
         ...
         open(60,file='heat.602',status='unknown')
         write(60,'(1x,f8.4,2x,f8.4,2x)') time/1.0d9,Pint_ice/1.0d12
    !!!     Pint_ice  = Hint_ice*rho_iceI*Volume_iceI
    !!!     Pint_core = Hint_core*rho_core*Volume_core
         Hint_ice = Pint_ice/rho_iceI/Volume_iceI
         Hint_core = Pint_core/rho_core/Volume_core
         ...
         open(70,file='heat.702',status='unknown')
         write(70,'(1x,f8.4,2x,f8.4,2x,f8.3,2x,f8.2,2x)') time/1.0d9,Pint_ice/1.0d12,Hint_ice*1.0d9,Volume_iceI/1.0d18
! Radiaogenic heat in the core (H_core), and heat flux at the top of the core following Kirk and Stevenson (1987) (F_core_KS)
         call Heatflux_top_core(time,H_core,P_core,F_core_KS,DT_core)
         open(40,file='heat.402',status='unknown')
         write(40,'(1x,f8.4,2x,d11.5,2x,f8.4,2x,f8.4)') time/1.0d9,H_core*rho_core,P_core/1.0d12,F_core_KS*1.0d3
         
! Temperature at bottom of iceI layer, and temperature jump
         call Temperature_bottom_iceI_shell(z_bot,pres,pCLA,Tbot,dTbotdr,DeltaT)

! Radial profile of temperature in the core calculated by solving heat diffusion equation (T_core) and  top heat flux (F_core_DE).
...
!    - Top boundary condition is set to Tbot at each time step
         ...
         F_core_DE = k_core*(-T_core(npc-2) + 4.0d0*T_core(npc-1) - 3.0d0*T_core(npc))/2.0d0/dr_core

         Prad_TW   = Hrad*Volume_core/1.0d12
         Ptidal_TW = Htidal*Volume_core/1.0d12
         open(50,file='heat.502',status='unknown')
         write(50,'(1x,f8.4,2x,f8.4,2x,f8.4,2x,f8.4)') time/1.0d9,Prad_TW,P_core/1.0d12,F_core_KS*1.0d3
...
!-----------------------------------------------------------------------------------------
      subroutine Heatflux_top_core(time,Hrad,power,flux,DTemp)
...
!---> Decay constants (in yr^-1)
      ...
      timeref   = 0.0d0
      do 5 i=1,nele
         C0(i) = C(i)*dexp(lambda(i)*timeref)  !! what is this equation means? 
5     continue

      surface = 4.0d0*pi*R_core**2.0d0
...
10    continue
      ...
      end subroutine Heatflux_top_core
